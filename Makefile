...
#                                       #   跨平台产物走交叉编译路径
#   make install-dev                    # macOS: 把 release 产物安装到 /Applications
#                                       #   （修补 ad-hoc 签名，消除 Gatekeeper 警告）
#   make clean-dmg                      # macOS: 清理 hdiutil 孤儿挂载
#                                       #   （cargo tauri build 报 "failed to run bundle_dmg.sh" 时用）
#
# 跨平台编译说明：
...
# ==============================================================================
#
# 把 release 产物安装到 /Applications/KITE.app, 同时修补 ad-hoc 签名,
# 消除 Tauri 2 默认签名导致的 Gatekeeper "无法验证是否恶意软件" 弹窗.
#
# 不依赖 Developer ID 证书 (公网分发仍需要; 详见 docs/operations/deploy-mac.md 待补).
# 仅 macOS 上可用, 其他平台直接报错退出.

install-dev: ## macOS: 安装 release 产物到 /Applications 并修补签名 (供开发期使用)
	@if [ "$(HOST_OS)" != "Darwin" ]; then \
		echo "ERROR: install-dev 必须在 macOS 上执行 (当前 HOST_OS=$(HOST_OS))." >&2; \
		exit 1; \
	fi
	@bash scripts/install-dev-mac.sh

# ==============================================================================
# dmg 残留清理 (开发期反复跑 tauri build 时 hdiutil 偶尔会留下孤儿挂载)
# ==============================================================================
#
# 背景: Tauri 2.x bundle_dmg.sh 用 hdiutil create -attach 在 /Volumes/dmg.<random>
# 下挂载一个 sparse read-write 镜像, 写入 KITE.app, 卸载, 转 UDZO 压缩.
# 中间任何步骤失败 (AppleScript 卡住, 上次没卸载干净) 都会留下孤儿挂载.
# 下次再跑 cargo tauri build 时 hdiutil create 又要 mount 到 /Volumes/dmg.<random>,
# 发现挂载点被占, create-dmg 报 'failed to run bundle_dmg.sh',
# create 失败后还会留一个 rw.<pid>.<name>.dmg 在 bundle/macos/.
#
# 复现路径:
#   1. cargo tauri build 中途 Ctrl-C / 强制退出 → /Volumes/dmg.* 没卸载
#   2. 磁盘满 (压缩阶段写盘失败)
#   3. 系统休眠 → hdiutil 链路 hang
#
# 用法:
#   make clean-dmg         # 卸载所有 /Volumes/dmg.*, 删 bundle/macos/rw.*.dmg
#
# 注意: 不会动最终的 KITE_*.dmg 和 KITE.app, 只清挂载点 + 中间 RW 镜像.

clean-dmg: ## macOS: 清理 bundle_dmg.sh 留下的孤儿挂载 (解决 "failed to run bundle_dmg.sh")
	@if [ "$(HOST_OS)" != "Darwin" ]; then \
		echo "ERROR: clean-dmg 必须在 macOS 上执行 (当前 HOST_OS=$(HOST_OS))." >&2; \
		exit 1; \
	fi
	@echo "==> 卸载 /Volumes/dmg.* 残留挂载"
	@for m in $$(ls /Volumes 2>/dev/null | grep '^dmg\.' || true); do \
		echo "    detach /Volumes/$$m"; \
		hdiutil detach "/Volumes/$$m" >/dev/null 2>&1 || \
		diskutil unmount "/Volumes/$$m" >/dev/null 2>&1 || true; \
	done
	@echo "==> 删除 src-tauri/target/release/bundle/macos/rw.*.dmg 残留"
	@find src-tauri/target/release/bundle -maxdepth 2 -type f -name 'rw.*.dmg' -print -delete 2>/dev/null || true
	@echo "✓ dmg 残留清理完成"
